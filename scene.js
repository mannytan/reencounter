...
// section. Multiple cross sections are identical copies of the same
// animated shape, stacked vertically -- so all the position/geometry
// math below is computed exactly once per frame, and each cross
// section's meshes just share that same geometry/position data,
// parented under their own Group that carries the Y offset. Cross
// section 0 sits at CROSS_BASE_Y (the top); each following index
// stacks further down by crossSpread units.
...
// loop that follows the noise-driven drift every frame. One shared
// geometry (rebuilt when slice count changes) reused by every cross
// section's LineSegments instance.
const sliceLineMat = new THREE.LineBasicMaterial({ color: 0x4f8ef7 });
let sliceLineGeo;

function rebuildSliceLineGeometry(count) {
  if (sliceLineGeo) sliceLineGeo.dispose();
...
function rebuildRedPathGeometry(count) {
  if (redPathGeo) redPathGeo.dispose();
...
function rebuildOrangePathGeometry(count) {
  if (orangePathGeo) orangePathGeo.dispose();
...
// --- Cross Sections ---
// Per-cross-section instances (Object3D wrappers) sharing the geometry
// above. crossRadiusCubes/crossSliceCircles/crossSegmentCircles are
// arrays of arrays (one inner array of per-slice instances per cross
// section); everything else is one instance per cross section.
let crossGroups = [];
let crossRedGroups = [];
...
function rebuildCrossSections() {
  ...
  clearCrossSections();
  rebuildSliceLineGeometry(slices);
  rebuildRedPathGeometry(slices);
  rebuildOrangePathGeometry(slices);
...
function updateRadiusCubePositions(t) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Every cross section shares this same XZ shape -- only their
    // parent group's Y offset differs.
    for (let c = 0; c < crossRadiusCubes.length; c++) {
      crossRadiusCubes[c][i].position.set(x, 0, z);
      crossSliceCircles[c][i].position.set(x, 0, z);
      crossSliceCircles[c][i].scale.setScalar(clamped);
    }
  }
  ...
  const linePos = sliceLineGeo.attributes.position;
  for (let i = 0; i < count; i++) {
    ...
    for (let c = 0; c < crossSegmentCircles.length; c++) {
      crossSegmentCircles[c][i].position.set(midX, 0, midZ);
      crossSegmentCircles[c][i].scale.setScalar(segLength / 2);
    }
  ...
  redPathGeo.setPositions(redPathFlat);
  if (crossRedPaths.length) crossRedPaths[0].computeLineDistances();
  ...
  orangePathGeo.setPositions(orangePathFlat);
  if (crossOrangePaths.length) crossOrangePaths[0].computeLineDistances();
}
