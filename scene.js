...
// scales a 2D simplex field sampled at (slice index, cross-section
// index), so spread varies per slice per level -- not a straight ramp
// or a value that only varies by cross section), so unlike the shared
// white circle, every other line's geometry is computed independently
// per cross section, every frame.
...
function updateCrossSectionGeometry(c, t) {
  ...
  for (let i = 0; i < count; i++) {
    // Spread is sampled from the same 2D simplex field every slice
    // shares, indexed by v = slice index, w = cross-section index --
    // an actual 2D field over the (slice, cross section) grid, rather
    // than a value that only varies by cross section (which looked
    // off for the lower cross sections).
    const v = i;
    const w = c;
    const spreadNoise = noise2D(v, w);
...
// ramp up the stack) and (spread + simplex(slice, c) * crossSpreadStep)
// for spread (a 2D field over slice and cross-section, not a ramp).
