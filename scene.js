...
// scales a 3D simplex field sampled at (slice index, cross-section
// index, time), so spread varies per slice per level and animates over
// time -- not a straight ramp or a value that only varies by cross
// section), so unlike the shared white circle, every other line's
// geometry is computed independently per cross section, every frame.
...
function updateCrossSectionGeometry(c, t) {
  ...
  for (let i = 0; i < count; i++) {
    // Spread is sampled from a 3D simplex field: v = slice index,
    // w = cross-section index, t = time -- a field over the (slice,
    // cross section) grid that also animates, rather than a value that
    // only varies by cross section.
    const v = i;
    const w = c;
    const spreadNoise = noise3D(v, w, t * params.noiseSpeed);
...
// ramp up the stack) and (spread + simplex3D(slice, c, time) *
// crossSpreadStep) for spread (an animated field over slice and
// cross-section, not a ramp).
